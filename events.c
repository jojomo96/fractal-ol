...
void move_window(t_data *data, int x_offset, int y_offset, double zoom_factor) {
...
void move_viewport(t_data *data, int x_offset, int y_offset) {
    move_window(data, x_offset, y_offset, 1); // No zoom adjustment
}

void adjust_zoom(t_data *data, double zoom_factor) {
    move_window(data, 0, 0, zoom_factor); // No viewport movement, just zoom
}
...
int key_press(int keycode, t_data *data) {
    ...
    if (keycode == ESC_KEY_MAC || keycode == ESC_KEY_LINUX)
        ...
        adjust_zoom(data, 0.9); // Zoom in
    else if (keycode == MINUS_KEY)
        adjust_zoom(data, 1 / 0.9); // Zoom out
    return 0;
}
